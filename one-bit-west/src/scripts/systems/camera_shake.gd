...
var recoil_rot_offset: float = 0.0
var recoil_pos_offset: Vector3 = Vector3.ZERO
var _prev_recoil_rot_offset: float = 0.0
...
func kick(angle_deg: float = 3.0, back_distance: float = 0.04) -> void:
	...
	recoil_rot_offset -= kick_rad
	recoil_pos_offset += basis * Vector3(randf_range(-0.01, 0.01), 0, back_distance)
...
func _process(delta: float) -> void:
	...
	rotation.x += recoil_rot_offset - _prev_recoil_rot_offset
	_prev_recoil_rot_offset = recoil_rot_offset
	...
	position = base_position + bob_offset + recoil_pos_offset
